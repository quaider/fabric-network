...
<pre style="color: red">
不要使用fabric-ca-*系列的镜像，它们像已经不会再推上docker hub了，可以使用fabric-*代替
</pre>
...
peer chaincode invoke -C cnabs -n mycc -c '{"Args":["invoke","add","user1","4"]}' $ORDERER_CONN_ARGS
```